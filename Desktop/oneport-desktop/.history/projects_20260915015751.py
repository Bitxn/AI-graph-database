...
REPO_SIZE_LIMIT_MB = 700


def measure_size_mb(path: str, cap_mb: float = REPO_SIZE_LIMIT_MB) -> tuple[bool, int]:
    """(over_limit, size_mb) for the code OnePort would actually map — same
    IGNORE_DIRS the watcher/build use, so vendored deps (node_modules, .git,
    venv…) don't count against the guideline. Short-circuits the moment it goes
    over the cap, so a giant folder isn't fully walked."""
    ...
    for dirpath, dirnames, filenames in os.walk(path):
        ...
        for f in filenames:
            try:
                ...
                pass
        if total > cap:
            return True, int(cap_mb)          # over — stop walking
    return False, round(total / (1024 * 1024))
...
def create_project(path: str, base: str = "main") -> dict:
    ...
    proj = {
        ...
        "last_verdict": None,
        "history": [],
        "context_ready": False,      # has the initial mind-map (manifest.json) been built?
        "context_building": False,   # is that build running right now?
    }
...
def set_context_state(pid: str, *, building: bool | None = None,
                      ready: bool | None = None) -> dict | None:
    """Flag whether the initial repo context (mind-map / manifest.json) is
    currently building, and whether it's ready."""
    def apply(p):
        if building is not None:
            p["context_building"] = bool(building)
        if ready is not None:
            p["context_ready"] = bool(ready)
        return p
    return _mutate(pid, apply)
